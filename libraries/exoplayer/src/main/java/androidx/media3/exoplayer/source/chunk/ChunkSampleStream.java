...
public class ChunkSampleStream<T extends ChunkSource>
  ...
  public boolean continueLoading(LoadingInfo loadingInfo) {
    ...
    if (idle != isIdleAt(loadPositionUs)) {
      checkArgument(scalableBase != null, "change idle state on an object with null scalableBaseLoader");
      sampleQueues[0].scheduleIdle(idle, scalableBase.getNextLoadPositionUs());
      pendingResetPositionUs = idle ? C.TIME_UNSET : scalableBase.getNextLoadPositionUs();
    }
    ...
    if (isMediaChunk(loadable)) {
      ...
      if (pendingReset) {
        ...
        if (mediaChunk.startTimeUs != pendingResetPositionUs) {
          ...
          sampleQueues[1].setStartTimeUs(pendingResetPositionUs);
        }
        pendingResetPositionUs = C.TIME_UNSET;
      }
  ...
  public long getNextLoadPositionUs() {
    if (isPendingReset()) {
      ...
      return (loadingFinished || isIdleNext() || mediaChunks.isEmpty()) ? C.TIME_END_OF_SOURCE : getLastMediaChunk().endTimeUs;
    }
  }
  ...
  private boolean isIdle() {
    return sampleQueues[0].isIdle();
  }

  private boolean isIdleAt(long timeUs) {
    return sampleQueues[0].isIdleAt(timeUs);
  }

  private boolean isIdleNext() {
    return sampleQueues[0].isIdleAt(Long.MAX_VALUE);
  }
